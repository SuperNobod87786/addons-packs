# My Addon and Packs repository

## ⬇️ Downloads:
You can download any of the addons and packs from the [Releases Page](https://github.com/SuperNobod87786/addons-packs/releases).
## Index
- [EmpireCraftEnderman-rp](#empirecraftenderman-rp)
- [HardcoreEvent](#hardcoreevent)

## EmpireCraftEnderman-rp
A resource pack that changes the texture of the enderman to the EmpireCraft Enderman logo.  
  
**💬 EmpireCraft Discord:**  
Join the EmpireCraft realm discord:  
  
[![Discord](https://img.shields.io/badge/Discord-5865F2?style=for-the-badge&logo=discord&logoColor=FFFFFF)](https://discord.gg/8JyFrB3EB2)

## HardcoreEvent
A behavior pack that adds a custom play expirience to your server.  
**How it works:**  
Each player gets 3 hours of normal survival play time. After those 3 hours you will get set into hardcore mode, meaning dying will put you in spectator mode.
